...
Features
- `with_inkwell`: Pulls in the `inkwell` crate with the LLVM 19 backend feature enabled. Requires a compatible system LLVM installation and will compile native bindings (`llvm_sys`).
...
- Build and run tests with a system LLVM (example; you must install LLVM first):
  ...
  # Linux / macOS (example for LLVM 19)
  export LLVM_SYS_191_PREFIX=/usr
  cargo test -p codegen-llvm --features with_inkwell,real_llvm

  # Windows PowerShell (example)
  $env:LLVM_SYS_191_PREFIX = 'C:\Program Files\LLVM'
  cargo test -p codegen-llvm --features with_inkwell,real_llvm
...
- On Windows: install a compatible LLVM release and set `LLVM_SYS_<MAJOR>_PREFIX` to the install folder.
...
Helper scripts
- `scripts/setup-llvm.sh` — installs a compatible LLVM-19 toolchain on Debian/Ubuntu and prints the `LLVM_SYS_*` export to use.
...
- `scripts/setup-llvm.ps1` — PowerShell helper for Windows (prefers an existing `llvm-config`, otherwise installs/downloads a compatible LLVM and sets the session env var).
