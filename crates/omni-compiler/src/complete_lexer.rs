...
impl CompleteLexer {
    pub fn new(src: &str) -> Self {
        ...
        keywords.insert("byte".to_string(), TokenKind::Byte);
    ...
    /// Returns true when `text` is reserved as a keyword by the current v0.1.4
    /// compatibility lexer.  Formatters use this to re-emit semantic
    /// identifiers through the Edition-1 `r#identifier` escape when needed.
    pub fn is_keyword(text: &str) -> bool {
        Self::new("").keywords.contains_key(text)
    }
        ...
        while let Some(c) = self.peek_char() {
            ...
            if self.at_line_start {
                ...
                if self.nesting > 0 {
                    ...
                    let current = *self.indent_stack.last().ok_or_else(|| {
                        Diagnostic::error(
                            error_codes::LEXER_INCONSISTENT_INDENT,
                            "internal indentation stack became empty",
                        )
                    })?;
                    if indent > current {
                        ...
                        let restored = *self.indent_stack.last().ok_or_else(|| {
                            Diagnostic::error(
                                error_codes::LEXER_INCONSISTENT_INDENT,
                                "internal indentation stack became empty",
                            )
                        ...
                        if indent != restored {
                            return Err(Diagnostic::error(
                                error_codes::LEXER_INCONSISTENT_INDENT,
                                format!("Inconsistent indentation at line {}", self.line),
                            ));
                        }
                    }
                    self.at_line_start = false;
            ...
            // Line comments -- (Omni style), block comments --- ---, and operators -> / -
            if c == '-' {
                self.next_char();
                if let Some('-') = self.peek_char() {
                    ...
                    if let Some('-') = self.peek_char() {
                        ...
                        // Flush any remaining hyphens
                        for _ in 0..hyphen_count {
                            text.push('-');
                        }
                        if !closed {
                            return Err(Diagnostic::error(
                                error_codes::LEXER_UNTERMINATED_COMMENT,
                                "Unterminated legacy block comment",
                            ));
                        }
                        tokens.push(Token {
            ...
            // Edition-1 interpolated strings.  v0.1.x also accepts the
            // historical `{expr}` spelling inside f-strings; the formatter emits
            // the canonical `${expr}` spelling.  `d"..."` from the pre-Edition-1
            // research grammar is deliberately not reintroduced because the
            // current AST has no debug-format mode and Edition-1 removed it.
            if c == 'f' && self.peek_n_char(1) == Some('"') {
                let start_line = self.line;
                let start_col = self.col;
                self.next_char(); // f
                    ...
                    match self.peek_char() {
                        ...
                        Some('\\') => {
                            ...
                            let esc = self.next_char().ok_or_else(|| {
                                Diagnostic::error(
                                    error_codes::LEXER_UNTERMINATED_STRING,
                                    "Unterminated escape in interpolated string",
                                )
                            })?;
                            match esc {
                                'n' => value.push('\n'),
                                ...
                                other => {
                                    return Err(Diagnostic::error(
                                        error_codes::LEXER_INVALID_ESCAPE,
                                        format!(
                                            "Unsupported interpolated-string escape \\{}",
                                            other
                                        ),
                                    ));
                                }
                        ...
                        Some('$') if self.peek_n_char(1) == Some('{') => {
                            value.push('$');
                            value.push('{');
                            self.next_char();
                            self.next_char();
            ...
            if c == '\'' {
                ...
                let ch = match self.peek_char() {
                    ...
                    Some('\'') => {
                        return Err(Diagnostic::error(
                            error_codes::LEXER_UNEXPECTED_CHAR,
                            "Empty char literal",
                        ))
                    }
                    None => {
                ...
                if self.peek_char() != Some('\'') {
                    return Err(Diagnostic::error(
                        error_codes::LEXER_UNTERMINATED_STRING,
                        "Char literal must have closing quote",
                    ));
                }
                self.next_char();
                tokens.push(Token {
            ...
            // Edition-1 raw strings and raw identifiers.
            //   r"text"       zero-hash raw string
            //   r#"text"#     hash-delimited raw string
            //   r#keyword      raw identifier (semantic text is `keyword`)
            if c == 'r' && matches!(self.peek_n_char(1), Some('"') | Some('#')) {
                let start_line = self.line;
                let start_col = self.col;
                self.next_char(); // r
                ...
                while self.peek_char() == Some('#') {
                    ...
                    if hash_count > 255 {
                        return Err(Diagnostic::error(
                            error_codes::LEXER_UNEXPECTED_CHAR,
                            "Raw string delimiter exceeds the Edition-1 limit of 255 '#' characters",
                        ));
                    }
                    self.next_char();
                }
                        ...
                        match self.peek_char() {
                            Some('"') => {
                                ...
                                for offset in 0..hash_count {
                                    if self.peek_n_char(offset + 1) != Some('#') {
                                        matches_close = false;
                                        break;
                                    }
                                }
                ...
                if hash_count == 1 {
                    ...
                    if id.is_empty() || id.chars().next().is_some_and(|ch| ch.is_ascii_digit()) {
                        return Err(Diagnostic::error(
                            error_codes::LEXER_UNEXPECTED_CHAR,
                            "Invalid raw identifier after 'r#'",
                        ));
                    }
                    tokens.push(Token {
                ...
                return Err(Diagnostic::error(
                    error_codes::LEXER_UNEXPECTED_CHAR,
                    "Raw identifier uses exactly one '#'; multiple hashes require a quoted raw string",
                ));
            }

            // Edition-1 byte literal b'...'. The semantic token text is a
            // decimal 0..255 value so later parser stages never have to infer
            // source encoding from a Unicode scalar.
            if c == 'b' && self.peek_n_char(1) == Some('\'') {
                let start_line = self.line;
                let start_col = self.col;
                self.next_char(); // b
                self.next_char(); // opening quote
                let value = match self.peek_char() {
                    Some('\\') => {
                        ...
                        match self.peek_char() {
                            ...
                            Some('x') => {
                                self.next_char();
                                let hi = self.next_char().ok_or_else(|| {
                                    Diagnostic::error(
                                        error_codes::LEXER_INVALID_ESCAPE,
                                        "Byte literal \\x escape requires two hex digits",
                                    )
                                })?;
                                let lo = self.next_char().ok_or_else(|| {
                                    Diagnostic::error(
                                        error_codes::LEXER_INVALID_ESCAPE,
                                        "Byte literal \\x escape requires two hex digits",
                                    )
                            ...
                            Some(other) => {
                                return Err(Diagnostic::error(
                                    error_codes::LEXER_INVALID_ESCAPE,
                                    format!("Unsupported byte escape \\{other}"),
                                ))
                            }
                            None => {
                                return Err(Diagnostic::error(
                    ...
                    Some(_) => {
                        return Err(Diagnostic::error(
                            error_codes::LEXER_UNEXPECTED_CHAR,
                            "Byte literal must contain one ASCII byte or \\xNN escape",
                        ))
                    }
                    None => {
                        return Err(Diagnostic::error(
                ...
                if let Some('"') = self.peek_char() {
                    ...
                    while let Some(ch) = self.peek_char() {
                        ...
                        if ch == '\\' {
                            ...
                            let byte = match self.peek_char() {
                                ...
                                Some('x') => {
                                    ...
                                    let hi = self.next_char().ok_or_else(|| {
                                        Diagnostic::error(
                                            error_codes::LEXER_INVALID_ESCAPE,
                                            "Byte string \\x escape requires two hex digits",
                                        )
                                    })?;
                                    let lo = self.next_char().ok_or_else(|| {
                                        Diagnostic::error(
                                ...
                                Some(other) => {
                                    return Err(Diagnostic::error(
                                        error_codes::LEXER_INVALID_ESCAPE,
                                        format!("Unsupported byte-string escape \\{other}"),
                                    ))
                                }
                                None => {
                                    return Err(Diagnostic::error(
                                        error_codes::LEXER_UNTERMINATED_STRING,
                                        "Unterminated byte-string escape",
                                    ))
                                }
                            };
                            bytes.push(byte);
                            ...
                            return Err(Diagnostic::error(
                                error_codes::LEXER_UNEXPECTED_CHAR,
                                "Non-ASCII byte-string content must use \\xNN escapes",
                            ));
                        }
                    }
            ...
            if c.is_ascii_digit() {
                ...
                while let Some(ch) = self.peek_char() {
                    if ch.is_ascii_digit() || ch == '_' {
                        ...
                        let next_c = self.peek_n_char(1);
                        if matches!(next_c, Some(next) if next.is_ascii_digit()) {
                            num.push('.');
                            self.next_char();
            ...
            let token_text: String = self.chars[start_pos..self.pos].iter().collect();
            tokens.push(Token {
                kind,
                text: token_text,
                line: start_line,
...
pub fn tokenize_complete(src: &str) -> Result<Vec<Token>, Diagnostic> {
