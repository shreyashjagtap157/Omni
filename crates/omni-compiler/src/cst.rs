...
pub struct SyntaxNode {
    ...
    /// Exact source snapshot for a root CST produced from source text.
    /// Child nodes remain a structured semantic-token projection; this root
    /// snapshot makes the v0.1.2 CST genuinely lossless for trivia/whitespace.
...
/// Build a CST while retaining the exact input bytes (as UTF-8 text) on the
/// root node. This is the historical v0.1.2 lossless-CST contract: tools can
/// inspect the structured token tree and can always recover the original
/// whitespace, comments, and delimiter spelling exactly.
pub fn build_cst_from_source(source: &str) -> Result<SyntaxNode, crate::diagnostics::Diagnostic> {
    let tokens = crate::complete_lexer::tokenize_complete(source)?;
    let mut root = build_cst(&tokens);
    root.source_text = Some(source.to_string());
    Ok(root)
}

/// Recover the exact source used to construct a lossless root CST.
pub fn recover_source(node: &SyntaxNode) -> Option<&str> {
