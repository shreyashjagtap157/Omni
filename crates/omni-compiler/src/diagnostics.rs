...
impl Diagnostic {
    ...
    /// Create a diagnostic from a raw error string with an optional span.
    /// Used for gradual migration of String-based error handling to typed diagnostics.
    pub fn from_string(msg: String, span: Option<Span>) -> Self {
        let mut diag = Diagnostic::error(error_codes::PARSER_UNEXPECTED_TOKEN, msg);
        if let Some(s) = span {
            diag = diag.with_span(s.clone()).with_label(Label::primary(s));
        }
        diag
    }
}
...
/// Serialize a diagnostic as JSON for tooling integration (LSP, CI, etc.).
pub fn emit_diagnostic_json(diag: &Diagnostic) -> String {
    let value = serde_json::json!({
        ...
        "suggestions": diag.suggestions.iter().map(|s| {
            serde_json::json!({
                "label": s.label,
                "text": s.text,
    ...
    serde_json::to_string_pretty(&value).unwrap_or_else(|_| diag.to_string())
}
...
pub mod error_codes {
    ...
    pub const LEXER_UNTERMINATED_COMMENT: DiagnosticCode = DiagnosticCode("1006");
    ...
    pub const TYPE_TRAIT_BOUND: DiagnosticCode = DiagnosticCode("4011");
    pub const TYPE_VISIBILITY_PRIVATE: DiagnosticCode = DiagnosticCode("4012");
    pub const TYPE_INVALID_CONSTRUCTOR: DiagnosticCode = DiagnosticCode("4013");
    pub const TYPE_BREAK_OUTSIDE_LOOP: DiagnosticCode = DiagnosticCode("4014");
    pub const TYPE_CONTINUE_OUTSIDE_LOOP: DiagnosticCode = DiagnosticCode("4015");
    pub const TYPE_MISSING_RETURN: DiagnosticCode = DiagnosticCode("4016");
    pub const TYPE_TOP_LEVEL_EXECUTABLE_WITH_MAIN: DiagnosticCode = DiagnosticCode("4017");
    ...
    pub const CODEGEN_INVALID_MIR: DiagnosticCode = DiagnosticCode("7003");
}
