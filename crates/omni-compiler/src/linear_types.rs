...
impl LinearTypeChecker {
    ...
    pub fn mark_consumed(&mut self, name: &str) -> Result<(), String> {
        if self.is_linear(name) {
            ...
            self.consumed
                .insert(name.to_string(), LinearState::Consumed);
        }
        Ok(())
    }

    pub fn check_available(&self, name: &str) -> Result<(), String> {
        if self.is_linear(name) && self.consumed.contains_key(name) {
...
impl InOutDesugar {
    ...
    pub fn generate_move_in(&self, param: &str) -> Vec<String> {
        ...
        vec![format!("let __in_{} = {};", real_name, param)]
    }

    pub fn generate_move_out(&self, param: &str) -> Vec<String> {
        let real_name = param.strip_prefix("inout_").unwrap_or(param);
        vec![format!("{} = __out_{};", real_name, real_name)]
    }
}

impl Default for InOutDesugar {
    fn default() -> Self {
        InOutDesugar::new()
    }
}
...
impl FfiSandbox {
    ...
    pub fn has_capability(&self, cap: &str) -> bool {
        self.io_caps.contains(&cap.to_string()) || self.fs_caps.contains(&cap.to_string())
    }
