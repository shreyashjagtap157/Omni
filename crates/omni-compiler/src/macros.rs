...
impl MacroDefinition {
    ...
    fn match_pattern(&self, pattern: &[MacroArg], args: &[MacroArg]) -> Option<HashMap<String, Vec<MacroArg>>> {
        ...
        for (p, a) in pattern.iter().zip(args.iter()) {
            match (p, a) {
                ...
                (MacroArg::Literal(name), _) if name.starts_with('$') => {
                    ...
                    bindings.entry(var_name.to_string()).or_insert_with(Vec::new).push(a.clone());
                }
                _ => return None,
            }
        }
    ...
    fn match_token_tree(&self, pattern: &[MacroArg], arg: &MacroArg, _bindings: &mut HashMap<String, Vec<MacroArg>>) -> bool {
        match (pattern, arg) {
            ([], MacroArg::TokenTree(tokens)) if tokens.is_empty() => true,
            ([MacroArg::Literal(lit)], MacroArg::Literal(a)) if lit == a => true,
            ([MacroArg::Repetition(_inner, kind)], MacroArg::TokenTree(tokens)) => {
                match kind {
    ...
    fn apply_template(&self, template: &[MacroArg], bindings: &HashMap<String, Vec<MacroArg>>) -> Vec<MacroArg> {
...
impl ComptimeContext {
    ...
    pub fn eval(&mut self, expr: &ComptimeExpr) -> Result<ComptimeValue, String> {
        ...
        match expr {
            ...
            ComptimeExpr::Call(name, args) => {
                self.call(name, args)
            }
            ComptimeExpr::Block(stmts) => {
                self.eval_block(stmts)
            }
            ComptimeExpr::TypeOf(t) => {
                Ok(ComptimeValue::Type(t.clone()))
            }
        }
    }
    
    fn bin_op(&mut self, op: &str, a: &ComptimeValue, b: &ComptimeValue) -> Result<ComptimeValue, String> {
        match (op, a, b) {
            ("+", ComptimeValue::Int(ai), ComptimeValue::Int(bi)) => Ok(ComptimeValue::Int(ai + bi)),
            ("-", ComptimeValue::Int(ai), ComptimeValue::Int(bi)) => Ok(ComptimeValue::Int(ai - bi)),
            ("*", ComptimeValue::Int(ai), ComptimeValue::Int(bi)) => Ok(ComptimeValue::Int(ai * bi)),
            ("/", ComptimeValue::Int(ai), ComptimeValue::Int(bi)) if *bi != 0 => Ok(ComptimeValue::Int(ai / bi)),
            ("%", ComptimeValue::Int(ai), ComptimeValue::Int(bi)) if *bi != 0 => Ok(ComptimeValue::Int(ai % bi)),
            ("==", _, _) => Ok(ComptimeValue::Bool(a.eq(b))),
            ("!=", _, _) => Ok(ComptimeValue::Bool(!a.eq(b))),
            ("<", ComptimeValue::Int(ai), ComptimeValue::Int(bi)) => Ok(ComptimeValue::Bool(ai < bi)),
            (">", ComptimeValue::Int(ai), ComptimeValue::Int(bi)) => Ok(ComptimeValue::Bool(ai > bi)),
            ("<=", ComptimeValue::Int(ai), ComptimeValue::Int(bi)) => Ok(ComptimeValue::Bool(ai <= bi)),
            (">=", ComptimeValue::Int(ai), ComptimeValue::Int(bi)) => Ok(ComptimeValue::Bool(ai >= bi)),
            ("&&", ComptimeValue::Bool(ai), ComptimeValue::Bool(bi)) => Ok(ComptimeValue::Bool(*ai && *bi)),
            ("||", ComptimeValue::Bool(ai), ComptimeValue::Bool(bi)) => Ok(ComptimeValue::Bool(*ai || *bi)),
    ...
    fn type_of_value(&self, v: &ComptimeValue) -> TypeInfo {
        match v {
            ComptimeValue::Int(_) => TypeInfo { name: "i64".to_string(), params: vec![], fields: vec![] },
            ComptimeValue::String(_) => TypeInfo { name: "String".to_string(), params: vec![], fields: vec![] },
            ComptimeValue::Bool(_) => TypeInfo { name: "bool".to_string(), params: vec![], fields: vec![] },
            ComptimeValue::List(_) => TypeInfo { name: "Vec".to_string(), params: vec![], fields: vec![] },
            ComptimeValue::Tuple(_) => TypeInfo { name: "Tuple".to_string(), params: vec![], fields: vec![] },
...
impl PatternMatcher {
    ...
    pub fn is_exhaustive(&self, root_type: &str) -> bool {
        match root_type {
            "bool" => {
                self.arms.iter().any(|(p, _)| matches!(p, Pattern::Bool(true))) &&
                self.arms.iter().any(|(p, _)| matches!(p, Pattern::Bool(false)))
            }
    ...
    fn match_pattern(&self, pattern: &Pattern, value: &PatternValue) -> Option<HashMap<String, PatternValue>> {
        match (pattern, value) {
