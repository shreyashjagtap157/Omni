...
pub fn parse_raw_program(path: &Path) -> Result<Program, Diagnostic> {
...
pub fn document_to_c_header(document: &ExportDocument) -> Result<String, String> {
    validate_stage0_binding_document(document)?;
...
pub fn document_to_python_module(document: &ExportDocument) -> Result<String, String> {
    ...
    writeln!(
        output,
        "\"\"\"Auto-generated Omni scalar-ABI Python bindings.\"\"\""
    )
    .map_err(|e| e.to_string())?;
    ...
    for item in &document.items {
        match item {
            ...
            ExportedItem::Enum(enm) => {
                writeln!(
                    output,
                    "# enum {} is unavailable in the v0.1.4 foreign-value ABI",
                    sanitize_c_identifier(&enm.name)
                )
                .map_err(|e| e.to_string())?;
...
fn validate_stage0_binding_document(document: &ExportDocument) -> Result<(), String> {
    for item in &document.items {
        match item {
            ExportedItem::Function(function) => {
                if !function.type_params.is_empty() {
                    return Err(format!(
                        "binding export for generic function '{}' is not qualified in v0.1.4",
                        function.name
                    ));
                }
                for param in &function.params {
                    validate_scalar_binding_type(param.type_name.as_deref(), false).map_err(
                        |reason| {
                            format!(
                                "binding export for '{}.{}' is unavailable: {reason}",
                                function.name, param.name
                            )
                        },
                    )?;
                }
                validate_scalar_binding_type(function.return_type.as_deref(), true).map_err(
                    |reason| {
                        format!(
                            "binding export for return type of '{}' is unavailable: {reason}",
                            function.name
                        )
                    },
                )?;
            ...
            ExportedItem::Struct(strukt) => {
                return Err(format!(
                    "binding export for struct '{}' requires the stable aggregate value ABI deferred to v0.1.4",
                    strukt.name
                ));
            }
            ExportedItem::Enum(enm) => {
                return Err(format!(
                    "binding export for enum '{}' requires the stable aggregate value ABI deferred to v0.1.4",
                    enm.name
                ));
            }
        }
...
fn validate_scalar_binding_type(type_name: Option<&str>, is_return: bool) -> Result<(), String> {
    let Some(raw) = type_name else {
        if is_return {
            return Ok(());
        }
        return Err("parameter type is missing".to_string());
    };
    let normalized = raw.trim().to_ascii_lowercase();
    match normalized.as_str() {
        "int" | "i64" | "isize" | "usize" | "bool" => Ok(()),
        "void" | "unit" | "()" if is_return => Ok(()),
        _ => Err(format!(
            "type '{raw}' has no qualified v0.1.4 foreign ABI representation"
        )),
    }
}
...
fn c_type_for(type_name: Option<&str>, is_return_type: bool) -> &'static str {
    match type_name.map(|value| value.trim().to_ascii_lowercase()) {
        ...
        Some(ref value)
            if (value == "void" || value == "unit" || value == "()") && is_return_type =>
        {
            "void"
        }
