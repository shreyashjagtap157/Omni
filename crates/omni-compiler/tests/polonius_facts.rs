...
fn basic_defs_and_moves_emit_facts() {
    ...
    assert!(
        facts.iter().any(|s| s == "def main 0 0 a"),
        "expected def fact for a"
    );
    assert!(
        facts.iter().any(|s| s == "move main 0 1 a b"),
        "expected move fact for a->b"
    );
    assert!(
        facts.iter().any(|s| s == "use main 0 2 a"),
        "expected use fact for a"
    );
}
...
fn base_and_field_definitions_and_move() {
    ...
    let facts = polonius::generate_region_loan_facts(&module);
    assert!(
        facts.iter().any(|s| s == "def main 0 0 x"),
        "expected def fact for x"
    );
    assert!(
        facts.iter().any(|s| s == "def main 0 1 x.a"),
        "expected def fact for x.a"
    );
    assert!(
        facts.iter().any(|s| s == "move main 0 2 x z"),
        "expected move fact for x->z"
    );
}
