...
fn polonius_parity_additional_cases() {
    ...
    b1.instrs.push(mir::Instruction::ConstInt { dest: "x".to_string(), value: 1 });
    b1.instrs.push(mir::Instruction::Drop { var: "x".to_string() });
    b1.instrs.push(mir::Instruction::Print { src: "x".to_string() });
    f1.blocks.push(b1);
    ...
    b2.instrs.push(mir::Instruction::ConstInt { dest: "x".to_string(), value: 10 });
    b2.instrs.push(mir::Instruction::ConstInt { dest: "x.a".to_string(), value: 1 });
    b2.instrs.push(mir::Instruction::Move { dest: "z".to_string(), src: "x".to_string() });
    b2.instrs.push(mir::Instruction::Print { src: "x.a".to_string() });
    f2.blocks.push(b2);
    ...
    b3a.instrs.push(mir::Instruction::ConstInt { dest: "a".to_string(), value: 1 });
    b3a.instrs.push(mir::Instruction::Move { dest: "b".to_string(), src: "a".to_string() });
    let mut b3b = mir::BasicBlock::new(1);
    b3b.instrs.push(mir::Instruction::Print { src: "a".to_string() });
    f3.blocks.push(b3a);
    f3.blocks.push(b3b);
    ...
    b4.instrs.push(mir::Instruction::ConstInt { dest: "a".to_string(), value: 1 });
    b4.instrs.push(mir::Instruction::Move { dest: "b".to_string(), src: "a".to_string() });
    b4.instrs.push(mir::Instruction::ConstInt { dest: "a".to_string(), value: 2 });
    b4.instrs.push(mir::Instruction::Print { src: "a".to_string() });
    f4.blocks.push(b4);
    ...
    let have_polonius = polonius_available() && std::env::var("OMNI_FORCE_REAL_POLONIUS").ok().as_deref() == Some("1");
    if !have_polonius {
    ...
    for (name, module) in cases {
        ...
        if have_polonius {
            ...
            assert_eq!(mock_res.is_ok(), real_res.is_ok(), "parity mismatch for case {}: facts=\n{}", name, facts);
        } else {
