...
fn missing_effect_annotation_on_function_body() {
    ...
    let f_fn = Stmt::Fn {
        ...
        ret_type: None,
        effects: vec![],
        body: vec![Stmt::ExprStmt(Expr::Call("g".to_string(), vec![Expr::StringLit("hey".to_string())]))],
    };

    let prog = Program { stmts: vec![g_fn, f_fn] };
...
fn callee_annotated_but_caller_missing() {
    ...
    let f_fn = Stmt::Fn {
        ...
        ret_type: None,
        effects: vec![],
        body: vec![Stmt::ExprStmt(Expr::Call("g".to_string(), vec![Expr::StringLit("test".to_string())]))],
    };
