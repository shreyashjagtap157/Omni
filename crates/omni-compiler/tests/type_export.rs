...
fn export_types_produces_json_for_functions_and_structs() {
    let file = write_source("struct Point [x: int, y: int]\nfn add(a, b) -> int:\n    return 1\n");
    ...
    assert_eq!(
        function["params"].as_array().expect("params array").len(),
        2
    );
}
...
fn export_types_can_render_c_header() {
    let file = write_source("fn add(a: int, b: int) -> int:\n    return 1\n");
...
#[test]
fn aggregate_binding_export_fails_closed_before_layout_abi() {
    let file = write_source("struct Point [x: int, y: int]\n");

    let error = export_types_file(file.path(), TypeExportFormat::CHeader)
        .expect_err("aggregate binding export must remain unavailable before the layout ABI");
    assert!(error.contains("stable aggregate value ABI"));
}
...
fn abi_check_detects_breaking_function_changes() {
    ...
    assert!(diffs
        .iter()
        .any(|diff| diff.contains("parameter count changed")));
}
