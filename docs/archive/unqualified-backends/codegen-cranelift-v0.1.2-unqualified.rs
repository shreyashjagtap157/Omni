...
#[deprecated(note = "use render_lir_text")]
pub fn compile_lir_stub(module: &Module) -> String {
    render_lir_text(module)
}
...
pub fn run_lir_interpreter(module: &Module) -> Result<RunResult, String> {
    ...
    fn interp_fn(
        ...
        while ip < body.len() {
            match &body[ip] {
                ...
                LirInstr::Mod => {
                    ...
                    if b == 0 { return Err("remainder by zero".to_string()); }
                ...
                LirInstr::Ret => {
                    ...
                    for _ in 0..ret_count {
                        rets.push(stack.pop().ok_or("stack underflow returning from function")?);
                    }
                    rets.reverse();
                    return Ok(rets);
                }
                ...
                | LirInstr::LoadInd
                | LirInstr::StoreInd => {
                    return Err("aggregate/pointer LIR is not implemented in the v0.1.2 Cranelift development oracle".to_string());
                }
            }
            ip += 1;
        }
        ...
        if f.rets.is_empty() {
            ...
            Err(format!("function '{}' reached end without required return value", f.name))
        }
    }
...
mod cranelift_backend {
    ...
    extern "C" fn print_str_host(ptr: i64, len: i64) {
    ...
    fn validate_jit_subset(module: &Module) -> Result<(), String> {
        for func in &module.functions {
            for instr in &func.body {
                match instr {
                    LirInstr::Call(name) if name == "__register_effect_handler" => {
                        return Err("effect handlers are not implemented by the Cranelift development oracle".to_string());
                    }
                    LirInstr::Call(name) if name.starts_with("__omni_unsupported_") => {
                        return Err(format!("unsupported source feature reached Cranelift oracle: {name}"));
                    }
                    LirInstr::LoadOffset(_, _)
                    | LirInstr::StoreOffset(_, _)
                    | LirInstr::GetAddr(_)
                    ...
                    | LirInstr::LoadInd
                    | LirInstr::StoreInd => {
                        return Err("aggregate/pointer LIR is not implemented by the v0.1.2 Cranelift development oracle".to_string());
                    }
                    _ => {}
                }
            }
    ...
    pub fn compile_and_run_with_jit(module: &Module) -> Result<Vec<i64>, String> {
        ...
        jit_builder.symbol("print_str", print_str_host as *const u8);
        ...
        // Imported print_str (ptr: i64, len: i64)
        let mut sig_print_str = jit.make_signature();
        sig_print_str.params.push(AbiParam::new(types::I64));
        sig_print_str.params.push(AbiParam::new(types::I64));
        let print_str_id = jit
            .declare_function("print_str", Linkage::Import, &sig_print_str)
            .map_err(|e| e.to_string())?;
        func_id_map.insert("print_str".to_string(), print_str_id);
        sig_map.insert("print_str".to_string(), sig_print_str);

        // Only source-declared functions plus explicit print imports are available.
        // Future effect/runtime facilities must be implemented explicitly; no fake host helpers.
            ...
            if !has_cf {
                ...
                for instr in &f.body {
                    match instr {
                        ...
                        LirInstr::PrintStr(s) => {
                            let c_str = CString::new(s.as_str()).map_err(|_| "string literal contains interior NUL")?;
                            ...
                            let ps_id = *func_id_map
                                .get("print_str")
                                .ok_or_else(|| "print_str not declared".to_string())?;
                            let callee = jit.declare_func_in_func(ps_id, builder.func);
                            builder.ins().call(callee, &[ptr_val, len_val]);
                        }
                        LirInstr::Nop => {}
                        LirInstr::Jump(_) | LirInstr::CondJump { .. } => {
                            return Err("internal error: control-flow instruction reached straight-line Cranelift lowering".to_string());
                        }
                        LirInstr::LoadOffset(_, _)
                        | LirInstr::StoreOffset(_, _)
                        | LirInstr::GetAddr(_)
                ...
                let stack_delta = |idx: usize| -> Result<isize, String> {
                    match &body[idx] {
                        ...
                        LirInstr::PrintStr(_) => Ok(0),
                        LirInstr::Drop(_) => Ok(0),
                        LirInstr::Nop => Ok(0),
                        LirInstr::LoadOffset(_, _) => Ok(1),
                        LirInstr::StoreOffset(_, _) => Ok(-1),
                        LirInstr::GetAddr(_) => Ok(1),
                        LirInstr::AddOffset => Ok(-1),
                        LirInstr::LoadInd => Ok(0),
                        LirInstr::StoreInd => Ok(-2),
                    }
                };
                ...
                for (i, instr) in body.iter().enumerate() {
                    match instr {
                        ...
                        LirInstr::Store(slot) | LirInstr::Drop(slot) => {
                ...
                for (idx, instr) in body.iter().enumerate() {
                    ...
                    let stack_params = incoming[idx].ok_or_else(|| format!("missing stack-depth analysis for {}:{idx}", f.name))?;
                    ...
                    match instr {
                        LirInstr::Const(v) => {
                            ...
                            vstack.push(val);
                            if idx + 1 < n {
                                let td = incoming[idx + 1].ok_or_else(|| format!("missing successor stack-depth analysis for {}:{}", f.name, idx + 1))?;
                        ...
                        LirInstr::Jump(target) => {
                            ...
                            let td = incoming[*target].ok_or_else(|| format!("missing target stack-depth analysis for {}:{}", f.name, target))?;
                            let start = vstack.len().saturating_sub(td);
                            let mut args: Vec<Value> = vstack[start..].to_vec();
                        ...
                        LirInstr::CondJump { if_true, if_false } => {
                            ...
                            let td_t = incoming[*if_true].ok_or_else(|| format!("missing true-target stack-depth analysis for {}:{}", f.name, if_true))?;
                            let td_f = incoming[*if_false].ok_or_else(|| format!("missing false-target stack-depth analysis for {}:{}", f.name, if_false))?;
                            if td_t != td_f {
                                return Err(
                        ...
                        LirInstr::LoadOffset(slot, _offset) => {
                            let v = *var_map
                                .get(slot)
                                .ok_or_else(|| "invalid slot".to_string())?;
                            let val = builder.use_var(v);
                        ...
                        LirInstr::StoreOffset(slot, _offset) => {
                            let val = vstack.pop().ok_or("stack underflow during lowering")?;
                            let var = *var_map
                                .get(slot)
                                .ok_or_else(|| "invalid slot".to_string())?;
                        ...
                        LirInstr::AddOffset => {
                            let offset = vstack.pop().ok_or("stack underflow during lowering")?;
                            let base = vstack.pop().ok_or("stack underflow during lowering")?;
                        ...
                        LirInstr::LoadInd => {
                            let _addr = vstack.pop().ok_or("stack underflow during lowering")?;
                            let val = builder.ins().iconst(types::I64, 0);
                            vstack.push(val);
                            if idx + 1 < n {
                                let td = incoming[idx + 1].ok_or_else(|| format!("missing successor stack-depth analysis for {}:{}", f.name, idx + 1))?;
                        ...
                        LirInstr::StoreInd => {
                            let _addr = vstack.pop().ok_or("stack underflow during lowering")?;
                            let _val = vstack.pop().ok_or("stack underflow during lowering")?;
                            if idx + 1 < n {
                                let td = incoming[idx + 1].ok_or_else(|| format!("missing successor stack-depth analysis for {}:{}", f.name, idx + 1))?;
                                let start = vstack.len().saturating_sub(td);
                                let mut args: Vec<Value> = vstack[start..].to_vec();
        ...
        let entry_name = module
            ...
            .find(|f| f.name == "main")
            .map(|f| f.name.clone())
            .ok_or_else(|| "Cranelift JIT module has no 'main' entry function".to_string())?;
...
#[cfg(not(feature = "use_cranelift"))]
pub fn compile_and_run_with_jit(_module: &Module) -> Result<Vec<i64>, String> {
    Err("Cranelift JIT backend is disabled; rebuild with feature 'use_cranelift'".to_string())
}
