...
fn emit_c_function(function: &Function) -> Result<String, String> {
    ...
    for instr in &function.body {
        match instr {
            ...
            Instr::Lt => {
                out.push_str("  { long long rhs = stack[--sp]; long long lhs = stack[--sp]; stack[sp++] = (lhs < rhs) ? 1 : 0; }\n");
            }
            Instr::Gt => {
                out.push_str("  { long long rhs = stack[--sp]; long long lhs = stack[--sp]; stack[sp++] = (lhs > rhs) ? 1 : 0; }\n");
            }
            ...
            Instr::Not => {
                out.push_str("  { long long a = stack[--sp]; stack[sp++] = (a == 0) ? 1 : 0; }\n");
            }
            Instr::Load(slot) => {
            ...
            Instr::LoadOffset(slot, _offset) => {
                out.push_str(&format!("  stack[sp++] = slots[{}];\n", slot));
            }
            Instr::StoreOffset(slot, _offset) => {
                out.push_str(&format!("  slots[{}] = stack[--sp];\n", slot));
            }
            Instr::GetAddr(_slot) => {
            ...
            Instr::LoadInd => {
                out.push_str("  stack[--sp]; stack[sp++] = 0;\n");
            }
            Instr::StoreInd => {
                out.push_str("  sp -= 2;\n");
            }
...
/// LLVM never aliases another backend. If real LLVM support is not compiled
/// in, callers get an explicit availability error.
#[cfg(not(feature = "real_llvm"))]
pub fn compile_and_run_with_llvm(_module: &Module) -> Result<Vec<i64>, String> {
    Err("LLVM backend is disabled; enable the real LLVM feature set explicitly".to_string())
}
...
pub fn compile_and_run_with_llvm(_module: &Module) -> Result<Vec<i64>, String> {
    Err("with_inkwell_stub does not execute programs; install/configure LLVM and enable with_inkwell for the LLVM oracle".to_string())
}
