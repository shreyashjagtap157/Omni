    ...
    if std::env::var("OMNI_USE_POLONIUS").ok().as_deref() == Some("1") {
        ...
        let tmpdir = std::env::temp_dir().join(format!(
            ...
            std::time::SystemTime::now()
                ...
                .map_err(|error| format!("adapter: system clock predates Unix epoch: {error}"))?
                .as_millis()
        ));
        ...
        for (func, blocks) in blocks_by_func.into_iter() {
            let mut numbered = blocks
                .into_iter()
                .map(|block| {
                    block
                        .parse::<usize>()
                        .map(|number| (number, block.clone()))
                        .map_err(|error| {
                            format!("adapter: non-numeric block id '{}' in {}: {}", block, func, error)
                        })
                })
            ...
            for w in numbered.windows(2) {
                if let [(_, a), (_, b)] = w {
                    if let (Some(src), Some(tgt)) = (
                        ...
                        rel_lines
                            .get_mut("cfg_edge")
                            .expect("cfg_edge relation is initialized above")
                            .push(format!("{}\t{}", src, tgt));
                    }
                }
            }
