#!/usr/bin/env python3
"""Run a canonical native conformance manifest (v0.1.2 scalar by default)."""
...
DEFAULT_MANIFEST = ROOT / "conformance/native_scalar_v0_1_2/manifest.json"


def invoke(cmd: list[str]) -> subprocess.CompletedProcess[bytes]:
    # Conformance output is byte-oriented. v0.1.4 qualifies arbitrary Bytes
    # values, so decoding subprocess output as UTF-8 would make the harness
    # itself reject valid programs containing non-UTF-8 bytes.
    return subprocess.run(cmd, cwd=ROOT, text=False, capture_output=True, check=False)


def expected_stdout(case: dict) -> bytes:
    if "stdout_hex" in case:
        try:
            return bytes.fromhex(case["stdout_hex"])
        except ValueError as exc:
            raise ValueError(f"invalid stdout_hex for {case.get('name', '<unnamed>')}: {exc}") from exc
    return case.get("stdout", "").encode("utf-8")
...
def main() -> int:
    ...
    for case in manifest["cases"]:
        ...
        if mode == "run":
            ...
            ok = proc.returncode == case["exit"] and proc.stdout == expected_stdout(case)
            if not ok:
            ...
            with tempfile.TemporaryDirectory(prefix="omni-conformance-build-fail-") as td:
                artifact = Path(td) / "app"
                proc = invoke([str(omni), "build", str(path), "-o", str(artifact)])
            needle = case["stderr_contains"]
            stderr = stderr_text(proc)
            ok = proc.returncode != 0 and needle in stderr
            if not ok:
                print(f"FAIL {case['name']}: exit={proc.returncode} expected build stderr to contain {needle!r}; stderr={stderr!r}")
                failures += 1
            else:
                print(f"PASS {case['name']}")
            executed += 1
